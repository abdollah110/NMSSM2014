...
def _Return_SigBGData_Shape(Name, channel,cat,HistoName,PostFix,CoMEnergy,changeHistoName):
    if PostFix=="_TrgUncUp": PostFix==""
    if PostFix=="_TrgUncDown": PostFix==""
...
#    if cat=="_btag_high" and changeHistoName : cat = "_btagLoose_high"
    print "======> ", SubRootDir + "out_"+ Name +CoMEnergy+ '.root',     "    888888    ", channel+HistoName + cat+ PostFix
...
def MakeTheHistogram(channel,Observable,CoMEnergy,chl):
    ...
    #ScaleUp
    Table_FileUp = TFile("Yield"+CoMEnergy+"_SVMassUp.root")
    NormTableUp = Table_FileUp.Get('FullResults')
    #Norm
    Table_File = TFile("Yield"+CoMEnergy+"_SVMass.root")
    NormTable_ = Table_File.Get('FullResults')
    #ScaleDown
    Table_FileDown = TFile("Yield"+CoMEnergy+"_SVMassDown.root")
    NormTableDown = Table_FileDown.Get('FullResults')

    #TrgUp
    Table_FileTrgUp = TFile("Yield"+CoMEnergy+"_SVMass_TrgUncUp.root")
    NormTableTrgUp = Table_FileTrgUp.Get('FullResults')
    #TRGDown
    Table_FileTrgDown = TFile("Yield"+CoMEnergy+"_SVMass_TrgUncDown.root")
    NormTableTrgDown = Table_FileTrgDown.Get('FullResults')
    ...
    NormTable=[NormTableDown,NormTable_,NormTableUp,NormTableTrgUp,NormTableTrgDown]
...
#    SysTableHighpt= [Histo_HighPtRWUp,Histo_HighPtRWDown]
    ...
    TauScaleOut = ["_CMS_scale_t_"+channel+CoMEnergy+"Down", "", "_CMS_scale_t_"+channel+CoMEnergy+"Up", "_CMS_trg_mu_"+channel+CoMEnergy+"Up", "_CMS_trg_mu_"+channel+CoMEnergy+"Down"]
...
#    TH1.AddDirectory(0)
    ...
    for NameCat in category:
        ...
        for tscale in range(len(TauScale)):
            Observable=XXX
            if tscale==3:
                Observable= Observable+"_TrgUncUp"
            if tscale==4:
                Observable= Observable+"_TrgUncDown"
           ################################################
           #   Filling Signal
           ################################################
            for sig in range(len(signal)):
                for m in range(len(mass)):#    for m in range(110, 145, 5):
                    ...
                    print "Now is processing", signal[sig],mass[m], "   and the sacel is-------> ",TauScale[tscale]
                    tDirectory.cd()
...
#                    print Name, channel,NameCat, Histogram, TauScale[tscale],CoMEnergy,False
