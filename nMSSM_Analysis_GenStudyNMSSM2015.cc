...
//                if (isMu || isTot) {
                    ...
                    for (int i = 0; i < mu_.size(); i++) {
                        for (int k = 0; k < tau_.size(); k++) {
                            ...
                            if (BJETS.size() > 0 && hasMatchedTrigger && Trigger_MuTau_12(m) && MU_CUTS && TAU_CUTS && MuTau_Charge && MuTau_dR && secondMuVeto && thirdEleVeto && thirdMuVeto) {

                                plotFill("mutau", ++mutau, 20, 0., 20.);
                                
                                for (int gg=0; gg<genPar.size(); gg++) {
                                    
                                    if (fabs(genPar[gg].pdgId)== 15 && genPar[gg].status== 2  && deltaR2(genPar[gg],mu_[i]) < 0.2)
                                        plotFill("TauLepPTMatchedToMu",genPar[gg].pt , 200, 0., 200.);

                                    if (fabs(genPar[gg].pdgId)== 15 && genPar[gg].status== 2  && deltaR2(genPar[gg],tau_[k]) < 0.2)
                                        plotFill("TauLepPTMatchedToTau",genPar[gg].pt , 200, 0., 200.);
