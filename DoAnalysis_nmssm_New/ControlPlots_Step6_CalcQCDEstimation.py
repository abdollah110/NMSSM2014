...
Binning_OSSS = array.array("d",[0,1,2,3,4,5])
Binning_PT = array.array("d",[0,1,2,3,4,5])
...
# FIXME  just for btagging   if catName=="_btag": catLooseName="_btagLoose"
    ##  ooooooooooooooooooooooooooo   Bcategory change name  ooooooooooooooooooooooooooo
...
#    ShapeNum=GetShape_QCD(PostFix,CoMEnergy,channelName,catName,"_TauPt_LepAntiIso_mTLess30_OS_RelaxIso","")
    ShapeNum=GetShape_QCD(PostFix,CoMEnergy,channelName,catName,ObservableMMM+"_LepAntiIso_mTLess30_OS_RelaxIso","")
    HistoNum=ShapeNum.Get("XXX")
...
#    ShapeDeNum=GetShape_QCD(PostFix,CoMEnergy,channelName,catName,"_TauPt_LepAntiIso_mTLess30_SS_RelaxIso","")
    ShapeDeNum=GetShape_QCD(PostFix,CoMEnergy,channelName,catName,ObservableMMM+"_LepAntiIso_mTLess30_SS_RelaxIso","")
    HistoDeNum=ShapeDeNum.Get("XXX")
...
def ApplyCorrectionOnQCDShape(Observable,CoMEnergy, etaRange, catName, channelName, shift_TauFR, shift_OSSS):
    ...
    templateShape = TH1F("XXX", "", otherBin,Binlow,BinHigh)

    for bb in range(otherBin):
        NormInPtBin = 0
...
#        fakeCorrection_OSSS= fitFunc_Exp3Par(bb + 0.5, fitparOSSS0, fitparOSSS1,fitparOSSS2);
        ...
        templateShape.SetBinContent(bb+1, finalNorm)
        ...
        for channelName in channel:
            for shiftFR in CorrectionFR:
                for shiftOSSS in CorrectionOSSS:
                    print catName,shiftFR,shiftOSSS
...
#    GetFinalQCDShapeNorm("_TauPt","_8TeV")
#    GetFinalQCDShapeNorm("_TauEta","_8TeV")
    GetFinalQCDShapeNorm("_nBjets","_8TeV")
